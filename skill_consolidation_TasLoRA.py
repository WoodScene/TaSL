...
def get_lora_ipt(dataset_id, service_id, ipt_file_name, model_name):
    ...
    #print(len(ipt_present_dic))
    #print(len(ipt_previous_dic))
    # count_greater_than_01 = sum(1 for value in ipt_list if value > 0.2)
    # print(f"大于 0.2 的数据个数：{count_greater_than_01}")
    # sys.exit(1)
    #print((df1['Module_Name'] == df2['Module_Name']).all())

    assert set(ipt_present_dic.keys()) == set(ipt_previous_dic.keys())

    # 这里也得生成一个新的累计到当前为止的重要性表格
...
def lora_averaging(checkpoint_name, present_thresholds, previous_thresholds, dataset_id, service_id, ipt_present_dic, ipt_previous_dic, lora_present_path, lora_previous_path, tau):
    ...
    dataset_order = get_dataset_order(dataset_id)
    #weighted_weights = {}
    weighted_weights = {key: torch.zeros_like(param).to('cuda:0') for key, param in checkpoint_present.items()}

    # print(checkpoint_present)

    for key in ipt_present_dic:
        ipt_score_present = ipt_present_dic[key]
        ipt_score_previous = ipt_previous_dic[key]
        
        exp_term_t_minus_1 = np.exp(ipt_score_previous / tau)
        exp_term_t = np.exp(ipt_score_present / tau)
        denominator = exp_term_t_minus_1 + exp_term_t
        
        coefficient_t_minus_1 = exp_term_t_minus_1 / denominator
        coefficient_t = exp_term_t / denominator
        
        rank_r = int(key.split("|||")[1])
        ori_loraA_name = key.split("|||")[0] + "lora_A.weight"
        ori_loraB_name = key.split("|||")[0] + "lora_B.weight"
        
        tensor_present_loraA = checkpoint_present[ori_loraA_name].to('cuda:0')
        tensor_present_loraB = checkpoint_present[ori_loraB_name].to('cuda:0')
        tensor_previous_loraA = checkpoint_previous[ori_loraA_name].to('cuda:0')
        tensor_previous_loraB = checkpoint_previous[ori_loraB_name].to('cuda:0')
        
        weighted_weights[ori_loraA_name][rank_r,:] = coefficient_t*tensor_present_loraA[rank_r,:] + coefficient_t_minus_1*tensor_previous_loraA[rank_r,:]
        weighted_weights[ori_loraB_name][:,rank_r] = coefficient_t*tensor_present_loraB[:,rank_r] + coefficient_t_minus_1*tensor_previous_loraB[:,rank_r]
...
def train(
    # model/data params
    dataset_id: int = 2, # 1 - 5  5次实验
    service_begin_id: int = 1, # 这个表示从哪个service开始训练，默认从头开始训练
    with_replay: bool = False, # 这个表示是否用有memory的数据集进行训练
    checkpoint_name: str = "",
    select_thresholds: float = -1, # 把importance score超过这个阈值的模块认为是重要的
    ipt_file_name: str = "",
    model_name: str = "",
    tau: float = 0.25, # 温度系数
):
    checkpoint_name = checkpoint_name + "_dataset_id_" + str(dataset_id)
    assert str(dataset_id) in checkpoint_name, "dataset_id 与 checkpoint_name 不匹配"
    assert model_name != ""
    print("adaptive fine-grained lora averaging begin!")
    ...
    # 这得获取一下权重，开始做averaging
    # 这个重写一下model.add_weighted_adapter这个函数
    ...
    lora_averaging(checkpoint_name, present_thresholds, previous_thresholds, dataset_id, service_id, ipt_present_dic, ipt_previous_dic, lora_present_path, lora_previous_path, tau)
