        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    # 在这里获取梯度
                    # print(f"step is {self.state.global_step} and {step}")
                    # 这两个step还不一样呢，以前面的为准吧 step is 6 and 27
                    if self.ipt_score is not None:
                        self.ipt_score.update_score(self.model, self.state.global_step)
