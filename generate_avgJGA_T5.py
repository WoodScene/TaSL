...
def main(args):
    ...
    if with_replay:
        if lora_type == "vanilla":
            model_path = os.path.join("./checkpoint_files", args.model_name + "_dataset_id_"+str(dataset_id)+"_with_memoryreplay", str(len(dataset_order)-1)+"-"+last_service_name)
        else:
            model_path = os.path.join("./checkpoint_files", args.model_name + "_importance_dataset_id_"+str(dataset_id)+"_averaging_with_memoryreplay", str(len(dataset_order)-1)+"-"+last_service_name+ "" + lora_type)
    else:
        if lora_type == "vanilla":
            model_path = os.path.join("./checkpoint_files", args.model_name + "_dataset_id_"+str(dataset_id), str(len(dataset_order)-1)+"-"+last_service_name)
        else:
            model_path = os.path.join("./checkpoint_files", args.model_name + "_importance_dataset_id_"+str(dataset_id)+"_averaging", str(len(dataset_order)-1)+"-"+last_service_name+ "" + lora_type)

    if args.model_path != "":
        model_path = os.path.join("./checkpoint_files", args.model_path + "_dataset_id_"+str(dataset_id)+"_averaging", str(len(dataset_order)-1)+"-"+last_service_name+ "" + lora_type)
    ...
    if with_replay:
        if lora_type == "vanilla":
            output_dir = os.path.join("./output", args.model_name + "_dataset_id_"+str(dataset_id)+"_avgJGA_with_memoryreplay")
        else:
            output_dir = os.path.join("./output", args.model_name + "_importance_dataset_id_"+str(dataset_id)+"_avgJGA_with_memoryreplay"+ lora_type)
    else:
        if lora_type == "vanilla":
            output_dir = os.path.join("./output", args.model_name + "_dataset_id_"+str(dataset_id)+"_avgJGA")
        else:
            output_dir = os.path.join("./output", args.model_name + "_importance_dataset_id_"+str(dataset_id)+"_avgJGA"+ lora_type)
    
    if args.output_dir != "":
        output_dir = os.path.join("./output", args.output_dir + "_dataset_id_"+str(dataset_id)+"_avgJGA"+ lora_type)
